...
By defining the angular velocity omega(t) = theta'(t), we obtain the system:
...
with theta(0) = pi - 0.1, and is initially at rest, 
so omega(0) = 0. Then the vector of initial conditions is'''


y0 = [np.pi - 0.1, 0.0]
'''We will generate a solution at 101 evenly spaced samples 
in the interval 0 <= t <= 10. So our array of times is:'''


t = np.linspace(0, 10, 101)
'''Call odeint to generate the solution. 
To pass the parameters b and c to pend, 
we give them to odeint using the args argument.'''


sol = odeint(temp, y0, t, args=(b, c))
